...
class GuiLayout(object):
    ...
    def setup_layout(self):
        ...
        self.top_frame = TopFrame(
            self.master, self.frame_image, width=width, height=height_frame // 20)
        self.top_frame.pack(side='top', fill='x', padx=5, pady=5)

        self.frame_left = LeftFrame(
            self.pwindow, self.frame_image, width=width_frame_left, height=height_frame)
    ...
    def _update_layout(self):
        ...
        if self.last_window_width is None or abs(current_width - self.last_window_width) > 10:
            ...
            new_width_frame_right = current_width - \
                new_width_frame_left - new_width_frame_image

            # Adjust the sashes of the PanedWindow
            self.pwindow.sash_place(0, new_width_frame_left, 0)
            self.pwindow.sash_place(
                1, new_width_frame_left + new_width_frame_image, 0)
...
class MenuBuilder:
    ...
    def show_dialog(self, title, text):
        ...
        txt = Text(window, wrap="word", width=60,
                   height=18, background="light gray")
        ...
        add_hyperlink(
            "hyper3", "https://github.com/erichnau/ApInsight.git", "10.0", "10.end")
    ...
    def save_project_as(self):
        if self.json_file:
            ...
            if file_path:
                ...
                project_data = self.project_data.load_project_from_json(
                    self.json_file)

                # Serialize the sections data
                sections_data = {}
                if len(self.frame_right.sections) > 0:
                    sections_data = self.serialize_sections(
                        self.frame_right.sections)
    ...
    def serialize_sections(self, sections):
        ...
        for name, info in sections.items():
            serialized_data[name] = {
                # Assuming this is a BooleanVar
                'select': info['select'].get(),
    ...
    def open_options_window(self):
        ...
        self.use_compiled_exe = self.config_manager.get_boolean_option(
            'Application', 'use_compiled_exe')
        ...
        # Adjusted size to accommodate new frame
        ...
        checkbutton_exe = tk.Checkbutton(
            options_window, variable=use_compiled_exe_var)
        checkbutton_exe.pack()

        info_button = tk.Button(
            options_window, text="Information", command=show_information)
        info_button.pack()

        # Frame for default greyscale values
        greyscale_frame = tk.Frame(
            options_window, borderwidth=2, relief="groove")
        greyscale_frame.pack(pady=10)

        label_greyscale = tk.Label(
            greyscale_frame, text="Default Greyscale Values for Section View")
        ...
        def confirm():
            ...
            self.config_manager.set_option(
                'Application', 'use_compiled_exe', str(new_use_compiled_exe))
        ...
        confirm_button = tk.Button(
            options_window, text="Confirm", command=confirm)
        confirm_button.pack()
        ...
        exit_button = tk.Button(
            options_window, text="Exit", command=exit_options)
        exit_button.pack()
...
class TopFrame(Frame):
    ...
    def create_widgets(self):
        self.zoom_button = Button(
            self, text="Zoom to Full Extent", command=self.zoom_to_full_extent, state='disabled')
        self.zoom_button.pack(side="left", padx=5, pady=5)

        self.draw_section_button = tk.Button(
            self, text="Draw Section", command=self.enable_draw_mode, state='disabled')
        self.draw_section_button.pack(side='left', padx=5, pady=5)

        self.measure_tool = tk.Button(
